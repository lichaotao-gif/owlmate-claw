...
export function StrategyCenter({onSignal}:{onSignal:(text:string)=>void}){
 ...
 useEffect(()=>{try{const raw=localStorage.getItem('owlmate-modules-v1');if(raw){const c=JSON.parse(raw);if(Array.isArray(c.ids)&&c.ids.every((id:unknown)=>typeof id==='string'&&modules.some(m=>m.id===id))&&[c.single,c.portfolio].every(n=>Number.isFinite(n)&&n>=1&&n<=100))setConfig(c)}}catch{setSaved('无法读取本机设置，暂用默认配置。')}},[]);
