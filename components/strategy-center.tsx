...
const modules = [
  ...
  },
  {
    ...
    name: '夏普比率周报',
    group: '表现分析',
    desc: '比较指定历史窗口内的风险调整表现。',
    needs: '待接入连续收益序列、无风险利率及统一计算窗口。',
  },
  {
    id: 'mc',
    name: '蒙特卡洛月报',
    group: '方案验证',
    desc: '计划模拟 1 万条路径，比较不同配置的结果分布。',
    needs: '待接入收益模型、波动率及资产相关性；目前不生成模拟报告。',
  },
  {
    id: 'ev',
    name: 'EV 评分监控',
    group: '收益评估',
    desc: '每月复核预期收益与损失，解释评分变化。',
    needs: '待定义评分口径、收益损失分布及估计数据。',
  },
  {
    id: 'kelly',
    name: '凯利参考仓位',
    group: '仓位工具',
    desc: '在风险预算与仓位上限内评估参考配置。',
    needs: '待接入收益分布估计和组合约束；未生成凯利仓位。',
  },
  {
    id: 'breakout',
    name: '突破与加仓提醒',
    group: '条件提醒',
    desc: '突破条件满足后生成候选信号，优先检查风控限制。',
    needs: '待定义阻力位、确认条件、加仓步长及退出规则。',
  },
  {
    id: 'etf1',
    name: '猫头鹰 ETF 1 号',
...
export function StrategyCenter({
  ...
  const [open, setOpen] = useState(false),
    ...
    [sample, setSample] = useState('22'),
    [scope, setScope] = useState('portfolio'),
    [result, setResult] = useState('');
  useEffect(() => {
    const timer = window.setTimeout(() => {
      try {
        const raw = localStorage.getItem('owlmate-modules-v1');
        if (raw) {
          ...
          if (
            ...
            [c.single, c.portfolio].every(
              (n) => Number.isFinite(n) && n >= 1 && n <= 100,
            )
          )
            setConfig(c);
        }
      } catch {
        setSaved('无法读取本机设置，暂用默认配置。');
      }
    }, 0);
  ...
  function save(next: Config) {
    ...
    setResult('');
    try {
      localStorage.setItem('owlmate-modules-v1', JSON.stringify(next));
      setSaved('已保存在本机 · 无后台定时任务');
    } catch {
      setSaved('仅当前页面生效，浏览器保存失败');
    }
  }
  function toggle(id: string) {
    save({
      ...config,
      ids: config.ids.includes(id)
        ? config.ids.filter((x) => x !== id)
        : [...config.ids, id],
    });
  }
  function simulate() {
    const n = Number(sample);
    if (!sample.trim() || !Number.isFinite(n) || n < 0 || n > 100) {
      setResult('请输入 0–100 的假设回撤百分比。');
      return;
    }
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="owl-dialog strategy-center-dialog">
          ...
          <div className="strategy-module-grid">
            {modules.map((m) => {
              const added = config.ids.includes(m.id);
              return (
                <article
                  key={m.id}
                  className={'strategy-module ' + (added ? 'added' : '')}
                >
                  <div>
                    <span>{m.group}</span>
                    <small>
          ...
          {config.ids.includes('drawdown') && (
            <section className="strategy-settings">
              ...
              {result && <output className="strategy-preview">{result}</output>}
            </section>
          )}
          <output className="strategy-save-status">
            {saved || '设置保存在当前浏览器；周报和月报尚未接入计算与调度。'}
          </output>
        </DialogContent>
      </Dialog>
    </>
